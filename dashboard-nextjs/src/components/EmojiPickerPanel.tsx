...
import { Search, Lock, Bot as BotIcon } from "lucide-react";
import { useEmojis } from "@/components/EmojiContext";
import { cn } from "@/lib/utils";
import emojiData from "@/data/emoji-data.json";

// Split out of EmojiPicker.tsx and lazy-loaded (see EmojiPicker.tsx's
// `dynamic(() => import("./EmojiPickerPanel"))`) so the ~250KB emoji dataset
// + grid rendering only get fetched/parsed the first time someone actually
// opens a picker, instead of being part of every page's initial JS bundle
// (EmojiPicker is imported on 6+ dashboard pages).
...
function twemojiUrl(emoji: UnicodeEmoji) {
  return `/twemoji/svg/${emoji.h}.svg`;
}
...
export default function EmojiPickerPanel({ onPick, hideTabs }: EmojiPickerPanelProps) {
  ...
  // This panel only ever exists in the DOM while the popover is open (parent
  // mounts it conditionally), so "on mount" is the same thing as "on open" —
  // no separate open-tracking/reset-on-close effect needed here.
  useEffect(() => {
                ...
                <>
                  ...
                  <div className="grid grid-cols-9 gap-0.5">
                    {searchResults.custom.map((emoji) => (
                      ...
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={emoji.url} alt={emoji.name} loading="lazy" decoding="async" className="h-[22px] w-[22px] object-contain" />
                      </button>
                    ))}
                    {searchResults.bot.map((emoji) => (
                      ...
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={botEmojiUrl(emoji)} alt={emoji.name} loading="lazy" decoding="async" className="h-[22px] w-[22px] object-contain" />
                      </button>
                    ))}
