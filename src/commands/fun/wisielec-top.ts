...
const C_ORANGE = COLORS.HANGMAN;   // '#E67E22'

// Horizontal column anchors
const RANK_CX  = PAD + 28;
const AV_LEFT  = PAD + 72;
...
export async function run({ interaction }: ICommandOptions): Promise<void> {
  ...
  if (!stats.length) {
    await interaction.editReply({ content: '📊 Nikt jeszcze nie zagrał w Wisielca na tym serwerze. Użyj `/wisielec`!' });
    return;
  }
  ...
  ctx.fillText('Wisielec — Ranking', IMG_W / 2, HDR_H / 2);
  ...
  for (let i = 0; i < stats.length; i++) {
    ...
    if (u.avatarUrl) {
      try {
        ...
        logger.warn(`wisielec-top: failed to load avatar for ${s.userId}: ${err}`);
    ...
    const gamesPlayed = s.gamesPlayed || s.wins + (s.losses || 0);
    const winRate = gamesPlayed > 0
      ? Math.round(((s.wins || 0) / gamesPlayed) * 100)
      : 0;
    ctx.fillStyle = C_MUTED;
    ctx.font      = '17px sans-serif';
    ctx.fillText(`${s.wins || 0} wyg. · ${winRate}% winrate`, TEXT_X, midY + 14);

    // ── Best streak (right-aligned) ───────────────────────────────────────────
    const bestStreak = s.bestStreak || s.currentStreak || 0;

    ctx.textAlign = 'right';
    ctx.fillStyle = C_ORANGE;
    ctx.font      = 'bold 30px sans-serif';
    ctx.fillText(`${bestStreak}🔥`, RIGHT_X, midY - 10);

    ctx.fillStyle = C_MUTED;
    ctx.font      = '14px sans-serif';
    ctx.fillText('najlepsza seria', RIGHT_X, midY + 16);
  ...
  // ── Send image ────────────────────────────────────────────────────────────
  const attachment = new AttachmentBuilder(canvas.toBuffer('image/png'), { name: 'wisielec-top.png' });
