...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub.setName('setup').setDescription('Konfiguruje kanał do wysyłania życzeń urodzinowych.')
  )
  .addSubcommand((sub) =>
    sub.setName('clear').setDescription('Usuwa kanał do wysyłania życzeń urodzinowych.')
  )
  .addSubcommand((sub) =>
...
async function handleClear(interaction: ChatInputCommandInteraction, guild: Guild): Promise<void> {
  ...
  if (!cfg) {
    await replyWithError(
      ...
      'Brak skonfigurowanego kanału do wysyłania życzeń urodzinowych.\n' +
        'Aby skonfigurować, uruchom `/config-birthday setup`.'
    );
    return;
  }
  ...
  await replyWithSuccess(
    ...
    'Usunięto kanał do wysyłania życzeń urodzinowych.\n' +
      'Aby skonfigurować ponownie, uruchom `/config-birthday setup`.'
  );
}
