...
describe('admin/config-greetings command', () => {
  beforeEach(() => jest.clearAllMocks());

  test('set: confirm without selection -> validation message', async () => {
    await jest.isolateModules(async () => {
      const { run } = await import('../../../../src/commands/admin/configGreetings');
  ...
  test('set: happy path -> saves and shows embed', async () => {
    await jest.isolateModules(async () => {
      const { GreetingsConfigurationModel } = await import(
        '../../../../src/models/GreetingsConfiguration'
      );
      (GreetingsConfigurationModel.findOneAndUpdate as jest.Mock).mockResolvedValue({});
      const { run } = await import('../../../../src/commands/admin/configGreetings');
      ...
      expect(GreetingsConfigurationModel.findOneAndUpdate).toHaveBeenCalledWith(
        { guildId: interaction.guild.id },
        { guildId: interaction.guild.id, greetingsChannelId: 'c1' },
  ...
  test('remove: no config -> error embed', async () => {
    await jest.isolateModules(async () => {
      ...
      (GreetingsConfigurationModel.findOne as jest.Mock).mockReturnValue({
  ...
  test('remove: config exists -> delete and success embed', async () => {
    await jest.isolateModules(async () => {
      ...
      (GreetingsConfigurationModel.findOne as jest.Mock).mockReturnValue({
        lean: () => ({ exec: () => Promise.resolve({ greetingsChannelId: 'c1' }) }),
      });
      const { run } = await import('../../../../src/commands/admin/configGreetings');
      const { interaction } = buildInteraction(['c1']);
      interaction.options.getSubcommand.mockReturnValue('remove');
      await run({ interaction, client: {} as any });
      expect(GreetingsConfigurationModel.deleteOne).toHaveBeenCalledWith({
        guildId: interaction.guild.id,
      });
      expect(interaction.editReply).toHaveBeenCalledWith(
        expect.objectContaining({ embeds: [expect.any(Object)] })
  ...
  test('show: configured channel missing -> error embed', async () => {
    await jest.isolateModules(async () => {
      ...
      (GreetingsConfigurationModel.findOne as jest.Mock).mockReturnValue({
        lean: () => ({ exec: () => Promise.resolve({ greetingsChannelId: 'cX' }) }),
      });
      const { run } = await import('../../../../src/commands/admin/configGreetings');
      const { interaction } = buildInteraction([]);
      interaction.options.getSubcommand.mockReturnValue('show');
